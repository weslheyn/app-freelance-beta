...
const STATUS=[
  ...
  {id:'desligado',     emoji:'⛔', label:'Desligado',         planilha:'DESLIGADO',   color:'#95a5a6'},
];
...
// ═══════════════════════════════════════════════════
// BUSCA / FILTROS DA TELA PRINCIPAL
// ═══════════════════════════════════════════════════
let EMP_SEARCH='';
let EMP_SETOR='TODOS';

(function injectSearchStyles(){
  if(document.getElementById('betaSearchStyles')) return;
  const st=document.createElement('style');
  st.id='betaSearchStyles';
  st.textContent=`
    .emp-search-wrap{padding:10px 16px 0;display:grid;grid-template-columns:1.3fr .9fr;gap:10px}
    .emp-search-box,.emp-setor-box{position:relative}
    .emp-search-box .icon,.emp-setor-box .icon{position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:18px;opacity:.9;pointer-events:none}
    .emp-search-input,.emp-setor-select{width:100%;height:40px;border-radius:15px;border:1px solid var(--border);background:var(--s2);color:var(--text);padding:0 12px 0 38px;font-size:14px;outline:none}
    .emp-setor-select{appearance:none;-webkit-appearance:none;padding-right:34px}
    .emp-setor-box .arrow{position:absolute;right:12px;top:50%;transform:translateY(-50%);font-size:12px;color:var(--sub);pointer-events:none}
    .emp-empty-filter{margin:18px 16px;padding:18px;border:1px dashed var(--border);border-radius:16px;color:var(--sub);text-align:center;font-weight:700}
    .badge-desligado{background:#95a5a622;color:#dfe6e9;border-color:#95a5a655}
    .desligado-card{border-left:3px solid #95a5a6 !important;opacity:.9}
    .desligado-card .desligado-info{margin-top:10px;background:var(--s2);border:1px solid var(--border);border-radius:12px;padding:12px;color:var(--sub);line-height:1.45}
    .desligado-card .desligado-info strong{color:var(--text)}
    .desligado-card .reativar-btn{margin-top:10px;width:100%;background:none;border:1.5px solid #27ae60;color:#27ae60;border-radius:10px;padding:10px;font-size:12px;font-weight:700;cursor:pointer}
    @media (max-width:480px){.emp-search-wrap{grid-template-columns:1fr;gap:8px}}
  `;
  document.head.appendChild(st);
})();

function _fmtIsoBr(iso){if(!iso) return ''; const p=String(iso).split('-'); return p.length===3?`${p[2]}/${p[1]}/${p[0]}`:iso;}
function _calcFimDesligamento(startDate){
  const d=new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
  if(d.getDate()<=20) return new Date(d.getFullYear(), d.getMonth(), 20);
  return new Date(d.getFullYear(), d.getMonth()+1, 20);
}
function _getSearchBoxHtml(turnos){
  const setores=['TODOS', ...turnos.map(t=>t.label)];
  return `<div class="emp-search-wrap">
    <div class="emp-search-box"><span class="icon">🔍</span><input class="emp-search-input" id="empSearchInput" placeholder="Buscar colaborador..." value="${EMP_SEARCH.replace(/"/g,'&quot;')}"></div>
    <div class="emp-setor-box"><span class="icon">🏷️</span><select class="emp-setor-select" id="empSetorSelect">${setores.map(s=>`<option value="${s}"${EMP_SETOR===s?' selected':''}>${s==='TODOS'?'Todos os setores':s}</option>`).join('')}</select><span class="arrow">▾</span></div>
  </div>`;
}
function _activateSearchUi(){
  const inp=document.getElementById('empSearchInput');
  const sel=document.getElementById('empSetorSelect');
  if(inp && !inp.dataset.bound){
    inp.dataset.bound='1';
    inp.addEventListener('input',()=>{EMP_SEARCH=inp.value||''; buildMain();});
  }
  if(sel && !sel.dataset.bound){
    sel.dataset.bound='1';
    sel.addEventListener('change',()=>{EMP_SETOR=sel.value||'TODOS'; buildMain();});
  }
}
function _matchesEmpFilter(f, setorLabel){
  if(EMP_SETOR!=='TODOS' && setorLabel!==EMP_SETOR) return false;
  const term=(EMP_SEARCH||'').trim().toLowerCase();
  if(!term) return true;
  const hay=[f.nome,f.categoria,f.turno,setorLabel].filter(Boolean).join(' ').toLowerCase();
  return hay.includes(term);
}
function marcarDesligado(nome){
  const inicio=WEEK_DATES[curDay];
  const fim=_calcFimDesligamento(inicio);
  const extra={ini:`${inicio.getFullYear()}-${String(inicio.getMonth()+1).padStart(2,'0')}-${String(inicio.getDate()).padStart(2,'0')}`,
               fim:`${fim.getFullYear()}-${String(fim.getMonth()+1).padStart(2,'0')}-${String(fim.getDate()).padStart(2,'0')}`};
  const cursor=new Date(inicio.getFullYear(),inicio.getMonth(),inicio.getDate());
  while(cursor<=fim){
    setRecByDate(nome,cursor.getFullYear(),cursor.getMonth()+1,cursor.getDate(),'desligado',extra);
    cursor.setDate(cursor.getDate()+1);
  }
  showToast('⛔ Desligamento aplicado até '+_fmtIsoBr(extra.fim));
}
function reativarDesligado(nome, iniIso, fimIso){
  const ini=new Date(iniIso+'T00:00:00');
  const fim=new Date(fimIso+'T00:00:00');
  const cursor=new Date(ini);
  while(cursor<=fim){
    const rec=getRecByDate(nome,cursor.getFullYear(),cursor.getMonth()+1,cursor.getDate());
    if(rec && rec.status==='desligado') setRecByDate(nome,cursor.getFullYear(),cursor.getMonth()+1,cursor.getDate(),'',{});
    cursor.setDate(cursor.getDate()+1);
  }
  buildMain();refreshPendBadge();buildSummary();
  showToast('♻️ Funcionário reativado');
}
...
const FB_CONFIG_DEFAULT = {
  ...
  authDomain: "coco-bambu-presenca-beta.firebaseapp.com",
  databaseURL: "https://coco-bambu-presenca-beta-default-rtdb.firebaseio.com",
  projectId: "coco-bambu-presenca-beta",
  storageBucket: "coco-bambu-presenca-beta.firebasestorage.app",
...
function buildSummary(){
  const cnt={presente:0,falta:0,atestado:0,'troca-horario':0,'troca-folga':0,medida:0,'saida-antecipada':0,'banco-horas':0,'desligado':0};
  ...
  document.getElementById('summary').innerHTML=
    ...
    '<div class="scard"><div class="scard-num" style="color:#95a5a6">'+cnt['desligado']+'</div><div class="scard-lbl">Deslig.</div></div>';
}
...
function buildMain(){
  ...
  const blocos=turnos.map(t=>buildTurno(t)).filter(Boolean);
  document.getElementById('mainContent').innerHTML=_getSearchBoxHtml(turnos)+(blocos.length?blocos.join('<div style="height:8px"></div>'):'<div class="emp-empty-filter">Nenhum colaborador encontrado para esse filtro</div>');
  _activateSearchUi();
}

function buildTurno(t){
  const membrosFiltrados=t.membros.filter(f=>_matchesEmpFilter(f,t.label));
  if(!membrosFiltrados.length) return '';
  const cards=membrosFiltrados.map(f=>{
...
function buildCard(f){
  ...
  if(st==='desligado'){
    const exD=(r.extra&&typeof r.extra==='object')?r.extra:{};
    return '<div class="emp-card desligado-card" id="card_'+f.nome.replace(/ /g,'_')+'">'+
      '<div class="emp-top">'+
        '<div><button class="emp-name-btn" onclick="openFicha(\''+f.nome.replace(/'/g,"\\'")+'\')"><div class="emp-name">'+f.nome+'</div></button><div class="emp-meta">'+f.categoria+' · '+(f.horIni||'')+(f.horFim?' às '+f.horFim:'')+'</div></div>'+
        '<div class="emp-badges"><span class="badge badge-desligado">⛔ DESLIGADO</span></div>'+
      '</div>'+
      '<div class="desligado-info"><strong>Período:</strong> '+_fmtIsoBr(exD.ini||'')+' até '+_fmtIsoBr(exD.fim||'')+'<br><span style="font-size:12px">Oculto da operação até a próxima escala.</span></div>'+
      '<button class="reativar-btn" onclick="reativarDesligado(\''+f.nome.replace(/'/g,"\\'")+'\',\''+(exD.ini||'')+'\',\''+(exD.fim||'')+'\')">♻️ Reativar</button>'+
    '</div>';
  }
...
function selectStatus(name,newSt){
  ...
  if(status==='falta'||status==='atestado'||status==='troca-folga'){
...
function editTrocaFolga(name){
  // Limpa extra para mostrar campos abertos
  const r = getRec(curDay,name);
  setRec(curDay, name, 'troca-folga', {});
  buildMain(); refreshPendBadge();
  // Scroll para o card
  setTimeout(()=>{
    const card=document.getElementById('card_'+name.replace(/ /g,'_'));
...
function openFolgaModalById(f, dayIdx, editMode, folgaKey){
  ...
  ['dom_pres_obs','dom_comp_valor','dom_comp_auth','dom_comp_obs',
   'dom_troca_dia','dom_troca_auth','dom_troca_obs','dom_falta_obs',
  ...
  if(folgaRec&&editMode){
    ...
    if(folgaRec.tipo==='compra'){
      ...
      document.getElementById('dom_troca_dia').value=folgaRec.trocaDia||'';
...
function saveDomFolga(){
  ...
  if(_domTipo==='presente'){
    ...
    // 1) DIA ATUAL: marca PRESENTE com obs TROCA FOLGA
    const obsHoje='TROCA FOLGA — folga em '+rec.trocaDiaFmt+(rec.obs?' | '+rec.obs:'');
    setRec(dayIdx,nome,'presente',{domTipo:'troca',folgaEm:rec.trocaDiaFmt,obs:obsHoje,folgaTrabalhada:true});
    bridgeSaveByDate(nome,dayIdx,'presente',{domTipo:'troca',folgaEm:rec.trocaDiaFmt,obs:obsHoje,folgaTrabalhada:true});

    // 2) DATA ESCOLHIDA: marca como TROCA-FOLGA
    const obsFolga='TROCA FOLGA — trabalhou em '+dateStr;
    const drKey='drec_'+nome+'_'+trocaDia;
    const drVal={status:'troca-folga',extra:{obs:obsFolga,trocaDe:dateStr},ts:Date.now()};
    save(drKey,drVal);
    ...
    const trocaRecord={
      ...
      trabalhouIso:workedIso, trabalhouFmt:fmt2(workedIso),
    ...
    showToast('✅ Hoje: PRESENTE · '+rec.trocaDiaFmt+': FOLGA');
