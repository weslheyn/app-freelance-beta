...
function getSearchBoxHtml(){
  return '<div style="padding:0 14px 10px"><div style="display:flex;align-items:center;gap:8px;background:#131726;border:1px solid #242a3d;border-radius:14px;padding:10px 12px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.02)"><span style="font-size:14px;opacity:.9">🔍</span><input id="searchInput" type="text" placeholder="Buscar colaborador..." value="'+(employeeSearchTerm||'').replace(/"/g,'&quot;')+'" style="flex:1;background:transparent;border:none;outline:none;color:#fff;font-size:14px;font-family:inherit"></div></div>';
}

function activateEmployeeSearch(){
  const input=document.getElementById('searchInput');
  if(!input) return;
  if(input._searchBound) return;
  input._searchBound=true;
  input.addEventListener('input', function(){
    employeeSearchTerm=this.value||'';
...
function applyEmployeeSearchFilter(){
  ...
  const cards=document.querySelectorAll('.emp-card[data-nome]');
  cards.forEach(card=>{
    const nome=(card.getAttribute('data-nome')||'').toLowerCase();
    card.style.display = (!term || nome.includes(term)) ? '' : 'none';
  });
  document.querySelectorAll('#mainContent .section').forEach(sec=>{
    const visible=sec.querySelector('.emp-card[data-nome]:not([style*="display: none"])');
...
function buildMain(){
  ...
  document.getElementById('mainContent').innerHTML=getSearchBoxHtml()+turnos.map(t=>buildTurno(t)).join('<div style="height:8px"></div>');
  activateEmployeeSearch();
}
...
function buildFolgaCard(f, dayIdx){
  ...
  return `<div class="emp-card has-status" data-nome="${String(f.nome).replace(/"/g,'&quot;')}" style="opacity:.55;${borderLeft}">
    <div class="emp-top">
      <div>
...
function buildTrocaFolgaCard(f, drRec){
  ...
  return `<div class="emp-card has-status" data-nome="${String(f.nome).replace(/"/g,'&quot;')}" style="opacity:.55;border-left:3px solid #1abc9c;">
    <div class="emp-top">
      <div>
...
function buildLicencaCard(f){
  return '<div class="emp-card" data-nome="'+f.nome.replace(/"/g,'&quot;')+'" style="opacity:.5;border-left:3px solid #2980b9"><div class="emp-top"><div><button class="emp-name-btn" onclick="openFicha(\''+f.nome.replace(/'/g,"\\'")+'\')"><div class="emp-name">'+f.nome+'</div></button><div class="emp-meta">'+f.categoria+'</div></div><div class="emp-badges"><span class="badge badge-licenca">🏥 LICENÇA</span></div></div></div>';
}
...
function buildCard(f){
  ...
  if(atestadoSalvo || faltaSalva || trocaFolgaSalva){
    ...
    return '<div class="emp-card" data-nome="'+f.nome.replace(/"/g,'&quot;')+'" id="card_'+f.nome.replace(/ /g,'_')+'" style="opacity:.75;border-left:3px solid '+corBorda+'">'+
      '<div class="emp-top">'+
  ...
  return '<div class="emp-card'+(st?' has-status st-'+st:'')+'" data-nome="'+f.nome.replace(/"/g,'&quot;')+'" id="card_'+f.nome.replace(/ /g,'_')+'">'+
    '<div class="emp-top">'+
