...
let _importPreviewSource = 'sheets';
...
async function abrirImportarEscala(){
  ...
  document.getElementById('imp_btn_exportar').style.display = 'none';
  document.getElementById('imp_btn_ler').style.display = '';
  _importPreview = null;
  _importPreviewSource = 'sheets';
...
async function lerEscalaSheets(){
  ...
  try{
    ...
    _montarPreviewImportacao(lidos,{sheetId,aba,source:'sheets'});
...
function _montarPreviewImportacao(lidos,{sheetId='',aba='',source='sheets'}={}){
  ...
  for(const lido of lidos){
    ...
    if(!match && nL.length>=8){
      for(const [nA,f] of mapAtual){
        if(nA.length>=8 && (nA.startsWith(nL)||nL.startsWith(nA))){ match=f; break; }
      }
    }
    if(!match){
  ...
  for(const [nA,f] of mapAtual){
    if(f.turno==='LICENÇA') continue;
    const casou = normsLidos.has(nA) || [...normsLidos].some(nL=>nL.length>=8&&nA.length>=8&&(nA.startsWith(nL)||nL.startsWith(nA)));
    if(!casou) removidos.push(f);
  }

  _importPreview = {lidos, adicionados, atualizados, inalterados, removidos, sheetId, aba, source};
  _importPreviewSource = source;
  _renderImportPreview();
}

function _escalaExportKey(aba){
  return (aba||'SEM_ABA').normalize('NFD').replace(/[̀-ͯ]/g,'').toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_+|_+$/g,'');
}

async function exportarEscalaParaFirebaseBeta(){
  if(!_importPreview || !_importPreview.lidos || !_importPreview.lidos.length){
    showToast('⚠️ Leia a planilha primeiro');
    return;
  }
  if(!fbDb || !fbConnected){
    showToast('⚠️ Conecte o Firebase primeiro');
    return;
  }
  const {lidos, aba, sheetId, source} = _importPreview;
  const key = _escalaExportKey(aba);
  const payload = {
    aba,
    key,
    sheetId: sheetId||'',
  ...
  try{
    await fbDb.ref('escala_exports/'+key).set(payload);
    await fbDb.ref('config/escala_exportada_atual').set({aba,key,total:lidos.length,exportedAt:payload.exportedAt,source:payload.source});
    localStorage.setItem('ultima_escala_exportada_beta', JSON.stringify({aba,key,total:lidos.length,exportedAt:payload.exportedAt}));
    showToast('☁️ Escala exportada para o app beta');
  }catch(e){
    console.error('exportarEscalaParaFirebaseBeta:',e);
    showToast('❌ Erro ao exportar: '+e.message);
  }
}

async function puxarEscalaExportadaBeta(){
  if(!fbDb || !fbConnected){
    showToast('⚠️ Conecte o Firebase primeiro');
    return;
  }
  try{
    const metaSnap = await fbDb.ref('config/escala_exportada_atual').once('value');
    const meta = metaSnap.val();
    if(!meta || !meta.key){
      showToast('⚠️ Nenhuma escala exportada encontrada no beta');
      return;
    }
    const snap = await fbDb.ref('escala_exports/'+meta.key).once('value');
    const data = snap.val();
    if(!data || !Array.isArray(data.equipe) || !data.equipe.length){
      showToast('⚠️ Exportação beta vazia');
      return;
    }
    document.getElementById('imp_preview').style.display='block';
    _montarPreviewImportacao(data.equipe,{sheetId:data.sheetId||'',aba:data.aba||meta.aba||'',source:'beta'});
    showToast('☁️ Escala beta carregada');
  }catch(e){
    console.error('puxarEscalaExportadaBeta:',e);
    showToast('❌ Erro ao puxar escala beta: '+e.message);
  }
}
...
function _renderImportPreview(){
  ...
  document.getElementById('imp_btn_aplicar').style.display = (adicionados.length||atualizados.length||removidos.length)?'':'none';
  document.getElementById('imp_btn_exportar').style.display = (adicionados.length||atualizados.length||inalterados.length)?'':'none';
}

// Aplica as mudanças na equipe
function aplicarImportacao(){
  if(!_importPreview) return;
  const {lidos, aba, adicionados, atualizados, removidos} = _importPreview;
  const ts = Date.now();
  ...
  if(fbDb && fbConnected){
    const key = _escalaExportKey(aba);
    fbDb.ref('escala_exports/'+key).set({aba,key,sheetId:_importPreview.sheetId||'',source:_importPreview.source||'sheets',total:lidos.length,exportedAt:Date.now(),exportedAtIso:new Date().toISOString(),equipe:lidos}).catch(()=>{});
    fbDb.ref('config/escala_exportada_atual').set({aba,key,total:lidos.length,exportedAt:Date.now(),source:_importPreview.source||'sheets'}).catch(()=>{});
  }
